...
export class MyContainer extends Container {
    ...
    sleepAfter = '5s'; // Sleep the container if no requests are made in this timeframe
...
function PathCreate(pathname: string, method: string): string {
    return `${pathname}_${method}`;
}
const listPath = [
    PathCreate('/status', 'GET')
];
export default {
    async fetch(
        request: Request,
        ...
        if ((PathCreate(pathname, request.method)) in listPath) {
